...
1. **Capture** — 16 kHz mono PCM using `VOICE_RECOGNITION` audio source.
2. **Speech engine** — local whisper.cpp `base.en` Q5_1 when installed; Android's preferably on-device recognizer as the zero-setup fallback.
...
- Whisper inference runs in the app process using a model stored in private app files.
- Drafts and audio are not persisted.
- The first model installation is a one-time network download with SHA-256 verification.
